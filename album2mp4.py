...
def do_process(folder, _cover, output, add_title, font, verbose):
    ...
    for song in sorted(glob(folder + '*.mp3')):
        ...
        descr.append("{0:0>2}:{1:0>2} ".format(int(time) / 60, int(time) % 60) + os.path.split(song.split('.mp3')[0])[-1])
        args = [executable,
                '-loop', '1',
                '-y',
                '-i', cover,
        ...
        if add_title:
            ...
            args += [
                ...
                'drawtext=fontfile={}:text={}:fontcolor=white:fontsize=96:box=1:boxcolor=black@0.5:boxborderw=10:x=(w-tw)/2:y=(h/PHI)+th'.format(
                    font, "{} - {}".format(mp3info['tracknumber'][0], mp3info['title'][0])
                )]
        args += [
                 '-c:v', 'libx264',
                 '-c:a', 'copy',
    ...
    out = subprocess.check_output([executable, '-y', '-i', all_files, '-c', 'copy', output], stderr=subprocess.STDOUT)
    if verbose:
        print(out)
    with open(output + '.txt', 'w') as descr_file:
        for line in descr:
            descr_file.write(line + os.linesep)
        descr_file.write("Generated with album2mp4 https://github.com/jonnoftw/album2mp4")
...
if __name__ == "__main__":
    ...
    parser.add_argument('-f', '--folder', metavar='folder', help='The folder containing the mp3 files', default=cwd)
